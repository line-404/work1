...
ppatient search_patientlist(char patientid[19]) { 
	ppatient p;
...
void add_patientlist() {
	...
	// 假设这些变量已经在函数头部声明了，如果没有请声明一下：
    // int switch_choice; 

    choice = get_int_range("Please choose the treatment method: 1 outpatient 2 emergency\n", 1, 2);//急诊，门诊择其一，若满则系统推荐改挂另一个
    
    switch (choice) {
    case 1:
        // 如果门诊满了
        if (list.outpatient_number >= 120) {
            printf("The number of outpatients exceeds the maximum capacity of the hospital.\n");
            ...
            if (list.emergencypatient_number >= 80) {
                ...
                // 如果急诊还有号，再询问患者是否切换
                int switch_choice = get_int_range("The emergency department is available. Do you want to switch to emergency? 1 Yes 0 No\n", 0, 1);
                if (switch_choice == 1) {
                    printf("Successfully switched to emergency.\n");
                    choice = 2; // 更新 choice 的值，改为急诊
                } else {
                    // 患者选择不切换，退出
                    return; 
                }
    ...
    case 2:
        // 如果急诊满了
        if (list.emergencypatient_number >= 80) {
            printf("The number of emergency patients exceeds the maximum capacity of the hospital.\n");
            ...
            if (list.outpatient_number >= 120) {
                ...
                // 如果门诊还有号，再询问患者是否切换
                int switch_choice = get_int_range("The outpatient department is available. Do you want to switch to outpatient? 1 Yes 0 No\n", 0, 1);
                if (switch_choice == 1) {
                    printf("Successfully switched to outpatient.\n");
                    choice = 1; // 更新 choice 的值，改为门诊
                } else {
                    // 患者选择不切换，退出
                    return;
                }
	...
	get_numeric_string_fixed("Please enter ID number:\n", patientid, 19);
	r=search_patientlist(patientid);
	if (r) {
...
void print_hospitalization_bill() {
	pregistration p;
	int i, record[9],key;
...
void modify_patient(char id) {
	...
	do {
		get_numeric_string_fixed("Please enter ID number:\n", patientid, 19);
	    p = search_patientlist(patientid);
		if (p) {
