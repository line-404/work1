...
void initial_today_patient( Date day);//0:00时初始化当天patientlist，释放昨日看诊链表(ppatientlist结构体，今日时间）   #######自动
...
ppatient search_patientlist(char[19]); //利用身份证查找病患信息并打印函数 （return该患者节点）
ppatient search_by_patientnumber();//利用病号查找病患信息并打印函数 （return该患者节点）
...
void modify_record();//医生录入或修改对应患者诊疗记录
void modify_patient( );//仅患者修改个人基本信息(病人一般不知道病号故用身份证查询）(身份证号）
